...
for i = 1:m
  ybin = zeros(num_labels,1);
  ybin(y(i)) = 1;
  ybinmap(i,:) = ybin';
  somak(i) = sum((-ybin .* log(H(i,:)')) - ((1 - ybin) .* log(1-H(i,:)')));
end

% Unregularized Cost function (J) 
Junreg = (1/m) * sum(somak);

% Regularized Cost function (J) 
Jreg = Junreg + ((lambda/(2*m)) * (sum(sum(Theta1(:,2:end) .^ 2)) + sum(sum(Theta2(:,2:end) .^ 2))));
